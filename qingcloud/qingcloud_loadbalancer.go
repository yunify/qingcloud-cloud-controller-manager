...
func (qc *QingCloud) EnsureLoadBalancerDeleted(ctx context.Context, clusterName string, service *v1.Service) error {
	loadBalancerName := qc.getQingCloudLoadBalancerName(service)
	klog.V(3).Infof("EnsureLoadBalancerDeleted(%v, %v)", clusterName, loadBalancerName)
	...
	klog.Infof("Try to delete loadBalancer by its id '%s'", *loadBalancer.LoadBalancerID)
	errDelLoadbalancer := qc.deleteLoadBalancer(*loadBalancer.LoadBalancerID)
	klog.Infof("Try to delete security group by its id '%s'", *loadBalancer.SecurityGroupID)
	errDelSecurityGrp := qc.DeleteSecurityGroup(loadBalancer.SecurityGroupID)
	if errDelLoadbalancer != nil {
		klog.Errorf("Delete loadBalancer '%s' err '%s' ", *loadBalancer.LoadBalancerID, errDelLoadbalancer)
		return errDelLoadbalancer
	}
	if errDelSecurityGrp != nil {
		klog.Errorf("Delete SecurityGroup '%s' err '%s' ", *loadBalancer.SecurityGroupID, errDelSecurityGrp)
	}
	klog.Infof("Delete loadBalancer '%s' in zone '%s'", loadBalancerName, qc.zone)

	return nil
}

func (qc *QingCloud) getQingCloudLoadBalancerName(service *v1.Service) string {
	return fmt.Sprintf("k8s_%s_%s", service.Name, cloudprovider.DefaultLoadBalancerName(service))
}
	...
	if len(listeners) > 0 {
		...
		for _, qyLbListerner := range listeners {
			...
			if intIndex(k8sTCPPorts, *qyLbListerner.ListenerPort) < 0 {
				...
				klog.V(1).Infof("This LB listener '%s' is not available any more for this LB, so need to delete it", *qyLbListerner.LoadBalancerListenerID)
				result = "update"
			}
		}
